...
- 团签`M9010 列举骑手`
  - 新增筛选`deleted`是否删除
  - 新增筛选`subscribe`订阅状态
  - 新增字段 `deleted_at`删除时间、`subscribeStatus`订阅状态
...
- `M8003 换电记录`
  - 筛选新增
    - `alternative ` 换电方案 0:全部 1:满电 2:非满电
    - `cabinetId` 选择电柜ID
    - `storeId` 选择门店ID
    - `serial` 搜索电柜编号
  - 列表字段新增显示
    - `alternative` 换电方案  `true`非满电 `false`满电, 只有`true`的时候才显示为`非满电`
    - `success` 换电是否成功 (放在第二列优先展示)
    - `full` 电柜换电满电仓位信息
    - `empty` 电柜换电空电仓位信息
- 管理员登录
  - 新增字段`permissions`权限列表
