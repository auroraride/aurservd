...
func (p *kaixin) Cabinets() ([]*ent.Cabinet, error) {
    return ent.Database.Cabinet.QueryNotDeleted().Where(cabinet.Brand(model.CabinetBrandKaixin.Value()), cabinet.Status(model.CabinetStatusNormal)).All(context.Background())
}
