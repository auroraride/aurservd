...
func (s *riderBusinessService) preprocess(serial string, bt business.Type) {
    ...
    if cab.UsingMicroService() {
        ...
        // 获取仓位信息
        var err error
        s.response.UUID, s.response.Index, err = NewIntelligentCabinet(s.rider).BusinessUsable(cab.Brand, bus, cab.Serial, sub.Model)
        if err != nil {
            snag.Panic(err)
        }
    } else {
    ...
    NewBusinessRider(s.rider).
        ...
        SetTask(func() (*model.BinInfo, *model.Battery, error) {
            ...
            if s.cabinet.UsingMicroService() {
                return NewIntelligentCabinet(s.rider).DoBusiness(s.cabinet.Brand, s.response.UUID, adapter.BusinessActive, s.subscribe, nil, s.cabinet)
            }
            return s.putout()
        }).
    ...
    go func() {
        err := snag.WithPanic(func() {
            ...
            NewBusinessRider(s.rider).
                ...
                SetTask(func() (*model.BinInfo, *model.Battery, error) {
                    if s.cabinet.UsingMicroService() {
                        return NewIntelligentCabinet(s.rider).DoBusiness(s.cabinet.Brand, s.response.UUID, adapter.BusinessContinue, s.subscribe, nil, s.cabinet)
                    }
                    return s.putout()
                }).
                Continue(req.ID)
    ...
    go func() {
        err := snag.WithPanic(func() {
            NewBusinessRider(s.rider).
                ...
                SetTask(func() (*model.BinInfo, *model.Battery, error) {
                    if s.cabinet.UsingMicroService() {
                        return NewIntelligentCabinet(s.rider).DoBusiness(s.cabinet.Brand, s.response.UUID, adapter.BusinessUnsubscribe, s.subscribe, s.battery, s.cabinet)
                    }
                    return s.putin()
                }).
                UnSubscribe(req.ID)
    ...
    go func() {
        err := snag.WithPanic(func() {
            NewBusinessRider(s.rider).
                SetTask(func() (*model.BinInfo, *model.Battery, error) {
                    if s.cabinet.UsingMicroService() {
                        return NewIntelligentCabinet(s.rider).DoBusiness(s.cabinet.Brand, s.response.UUID, adapter.BusinessPause, s.subscribe, s.battery, s.cabinet)
                    }
                    return s.putin()
                }).
                SetCabinet(s.cabinet).
