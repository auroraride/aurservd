...
func (s *enterpriseRiderService) AddSubscribeDays(req *model.RiderSubscribeAddReq, rid *ent.Rider) {
	// 查询骑手申请是否有未审批的
	info, _ := ent.Database.SubscribeAlter.QueryNotDeleted().Where(subscribealter.RiderID(rid.ID), subscribealter.StatusNEQ(model.Unreviewed)).First(s.ctx)
	if info != nil {
		snag.Panic("存在未审批的申请")
	}
...
func (s *enterpriseRiderService) SubscribeAlterList(req *model.ApplyReq, rid *ent.Rider) *model.PaginationRes {
