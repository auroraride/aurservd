...
type RiderListReq struct {
    ...
    Start    *string `json:"start"`    // 注册开始时间
    End      *string `json:"end"`      // 注册结束时间
    Modified *bool   `json:"modified"` // 是否被修改过

    // TODO 以下还未做
    Condition *uint8  `json:"condition"` // 用户状态 0:未使用 1:未开通 2:计费中 3:寄存中 4:已过期 5:暂停中 6:已退租 7:已欠费 8:未认证 9:未办理 10:即将到期 11:已禁用 12:黑名单
