...
func (*assetTransfer) TransferDetail(c echo.Context) (err error) {
	...
	return ctx.SendResponse(biz.NewAssetTransfer().TransferDetail(definition.AssetSignInfo{
		AssetManager: ctx.AssetManager,
		Employee:     ctx.Employee,
	}, req))
}
...
func (*assetTransfer) TransferDetailsList(c echo.Context) (err error) {
	...
	return ctx.SendResponse(biz.NewAssetTransfer().TransferDetailsList(definition.AssetSignInfo{
		AssetManager: ctx.AssetManager,
		Employee:     ctx.Employee,
	}, req))
}
...
// @Router	/warestore/v2/transfer/{id} [PUT]
// @Summary	修改资产调拨
// @Tags	AssetTransfer - 资产调拨
// @Accept	json
// @Produce	json
...
func (*assetTransfer) Modify(c echo.Context) (err error) {
	ctx, req := app.WarestoreContextAndBinding[definition.AssetTransferModifyReq](c)
	return ctx.SendResponse(biz.NewAssetTransfer().Modify(definition.AssetSignInfo{
		AssetManager: ctx.AssetManager,
		Employee:     ctx.Employee,
	}, req))
}
...
// @Router	/warestore/v2/transfer/cancel/{id} [PUT]
// @Summary	取消资产调拨
// @Tags	AssetTransfer - 资产调拨
// @Accept	json
// @Produce	json
...
func (*assetTransfer) TransferCancel(c echo.Context) (err error) {
	ctx, req := app.WarestoreContextAndBinding[model.AssetTransferDetailReq](c)
	return ctx.SendResponse(biz.NewAssetTransfer().TransferCancel(definition.AssetSignInfo{
		AssetManager: ctx.AssetManager,
		Employee:     ctx.Employee,
	}, req))
}
