...
// NewSearch 创建搜索
func NewSearch[T any](apiKey, datastream string, addresses []string) (*ElasticSearch[T], error) {
	es, err := New(apiKey, datastream, addresses)
	if err != nil {
		return nil, err
	}
	return &ElasticSearch[T]{
		Search: es.client.Search().Index(es.index),
		Options: &SearchOptions{
			Size: 999,
		},
	}, nil
...
func (s *ElasticSearch[T]) DoRequest(request *search.Request, options ...SearchOption) (output []*T) {
	...
	if err != nil {
		zap.L().Error("ELASTIC 搜索失败", log.Payload(request), zap.Error(err))
		return nil
	}
	...
	for _, hit := range hits {
		...
		if err != nil {
			zap.L().Error("ELASTIC 搜索结果解析失败", zap.Error(err), zap.String("source", string(hit.Source_)))
			continue
		}
