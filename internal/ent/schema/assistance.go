...
func (Assistance) Fields() []ent.Field {
    return []ent.Field{
        ...
        field.Uint8("status").Default(0).Comment("救援状态 0:等待接单 1:已接单 2:已拒绝 3:救援失败 4:救援成功"),
        ...
        field.String("breakdown_desc").Optional().Nillable().Comment("故障描述"),
        ...
        field.Float("distance").Optional().Nillable().Comment("救援距离"),
        field.String("reason").Optional().Comment("救援原因"),
        field.String("battery_photo").Optional().Comment("电池照片"),
        field.String("joint_photo").Optional().Comment("与用户合影"),
        field.Float("cost").Optional().Nillable().Comment("本次救援费用"),
