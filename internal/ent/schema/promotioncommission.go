...
func (PromotionCommission) Fields() []ent.Field {
	return []ent.Field{
		...
		field.Uint64("first_new_num").Default(0).Comment("一级新签人数"),
		field.Uint64("second_new_num").Default(0).Comment("二级新签人数"),
		field.Uint64("first_renew_num").Default(0).Comment("一级续费人数"),
		field.Uint64("second_renew_num").Default(0).Comment("二级续费人数"),
		field.Float("first_new_amount_sum").Default(0).Comment("一级新签金额"),
		field.Float("second_new_amount_sum").Default(0).Comment("二级新签金额"),
		field.Float("first_renew_amount_sum").Default(0).Comment("一级续费金额"),
		field.Float("second_renew_amount_sum").Default(0).Comment("二级续费金额"),
