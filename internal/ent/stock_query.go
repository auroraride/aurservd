...
func (sq *StockQuery) QueryBattery() *BatteryQuery {
...
func (sq *StockQuery) WithBattery(opts ...func(*BatteryQuery)) *StockQuery {
	query := (&BatteryClient{config: sq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (sq *StockQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Stock, error) {
	...
	if query := sq.withCity; query != nil {
		if err := sq.loadCity(ctx, query, nodes, nil,
			func(n *Stock, e *City) { n.Edges.City = e }); err != nil {
			return nil, err
		}
	}
...
func (sq *StockQuery) loadCity(ctx context.Context, query *CityQuery, nodes []*Stock, init func(*Stock), assign func(*Stock, *City)) error {
...
func (sq *StockQuery) loadSubscribe(ctx context.Context, query *SubscribeQuery, nodes []*Stock, init func(*Stock), assign func(*Stock, *Subscribe)) error {
	ids := make([]uint64, 0, len(nodes))
	nodeids := make(map[uint64][]*Stock)
	for i := range nodes {
...
func (sq *StockQuery) loadBattery(ctx context.Context, query *BatteryQuery, nodes []*Stock, init func(*Stock), assign func(*Stock, *Battery)) error {
	ids := make([]uint64, 0, len(nodes))
	nodeids := make(map[uint64][]*Stock)
	for i := range nodes {
...
func (sq *StockQuery) loadStore(ctx context.Context, query *StoreQuery, nodes []*Stock, init func(*Stock), assign func(*Stock, *Store)) error {
	ids := make([]uint64, 0, len(nodes))
	nodeids := make(map[uint64][]*Stock)
	for i := range nodes {
...
func (sq *StockQuery) loadCabinet(ctx context.Context, query *CabinetQuery, nodes []*Stock, init func(*Stock), assign func(*Stock, *Cabinet)) error {
	ids := make([]uint64, 0, len(nodes))
	nodeids := make(map[uint64][]*Stock)
	for i := range nodes {
...
func (sq *StockQuery) loadRider(ctx context.Context, query *RiderQuery, nodes []*Stock, init func(*Stock), assign func(*Stock, *Rider)) error {
	ids := make([]uint64, 0, len(nodes))
	nodeids := make(map[uint64][]*Stock)
	for i := range nodes {
...
func (sq *StockQuery) loadEmployee(ctx context.Context, query *EmployeeQuery, nodes []*Stock, init func(*Stock), assign func(*Stock, *Employee)) error {
	ids := make([]uint64, 0, len(nodes))
	nodeids := make(map[uint64][]*Stock)
	for i := range nodes {
...
func (sq *StockQuery) loadSpouse(ctx context.Context, query *StockQuery, nodes []*Stock, init func(*Stock), assign func(*Stock, *Stock)) error {
	ids := make([]uint64, 0, len(nodes))
	nodeids := make(map[uint64][]*Stock)
	for i := range nodes {
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "stock_spouse" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
