...
func (pq *PersonQuery) QueryRider() *RiderQuery {
	query := &RiderQuery{config: pq.config}
...
func (pq *PersonQuery) FirstIDX(ctx context.Context) uint64 {
...
func (pq *PersonQuery) OnlyIDX(ctx context.Context) uint64 {
...
func (pq *PersonQuery) IDsX(ctx context.Context) []uint64 {
	ids, err := pq.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (pq *PersonQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Person, error) {
	...
	if query := pq.withRider; query != nil {
		...
		for _, n := range neighbors {
			fk := n.PersonID
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "person_id" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "person_id" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.Rider = append(node.Edges.Rider, n)
		}
	}
