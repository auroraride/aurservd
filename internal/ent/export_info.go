...
func (es *EnterpriseStation) GetExportInfo() string {
	return es.Name
}
...
func (ei *ExportInfo) GetExportInfoData() string {
	...
	case enterprisestation.Table:
		m, _ = Database.EnterpriseStation.Query().Where(enterprisestation.ID(ei.id)).First(ctx)
	}
	if m == nil {
		return ""
	}
