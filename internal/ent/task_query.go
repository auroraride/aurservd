...
func (tq *TaskQuery) QueryRider() *RiderQuery {
	query := (&RiderClient{config: tq.config}).Query()
...
func (tq *TaskQuery) WithRider(opts ...func(*RiderQuery)) *TaskQuery {
	query := (&RiderClient{config: tq.config}).Query()
...
func (tq *TaskQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Task, error) {
	...
	if query := tq.withRider; query != nil {
		if err := tq.loadRider(ctx, query, nodes, nil,
			func(n *Task, e *Rider) { n.Edges.Rider = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (tq *TaskQuery) loadRider(ctx context.Context, query *RiderQuery, nodes []*Task, init func(*Task), assign func(*Task, *Rider)) error {
...
type TaskQueryWith string

var (
	TaskQueryWithRider TaskQueryWith = "Rider"
)

func (tq *TaskQuery) With(withEdges ...TaskQueryWith) *TaskQuery {
	for _, v := range withEdges {
		switch v {
		case TaskQueryWithRider:
			tq.WithRider()
		}
	}
	return tq
