...
func (sq *StatementQuery) QuerySubscribes() *SubscribeQuery {
...
func (sq *StatementQuery) QueryEnterprise() *EnterpriseQuery {
...
func (sq *StatementQuery) IDs(ctx context.Context) ([]uint64, error) {
	var ids []uint64
...
func (sq *StatementQuery) Clone() *StatementQuery {
	...
	return &StatementQuery{
		...
		withSubscribes: sq.withSubscribes.Clone(),
...
func (sq *StatementQuery) WithSubscribes(opts ...func(*SubscribeQuery)) *StatementQuery {
	query := &SubscribeQuery{config: sq.config}
	for _, opt := range opts {
		opt(query)
	}
	sq.withSubscribes = query
	return sq
}
...
func (sq *StatementQuery) WithEnterprise(opts ...func(*EnterpriseQuery)) *StatementQuery {
	query := &EnterpriseQuery{config: sq.config}
	for _, opt := range opts {
		opt(query)
	}
...
func (sq *StatementQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Statement, error) {
	...
	if query := sq.withSubscribes; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		nodeids := make(map[uint64]*Statement)
		for i := range nodes {
		...
		query.Where(predicate.Subscribe(func(s *sql.Selector) {
			s.Where(sql.InValues(statement.SubscribesColumn, fks...))
		}))
...
func (sq *StatementQuery) Modify(modifiers ...func(s *sql.Selector)) *StatementSelect {
...
// Modify adds a query modifier for attaching custom logic to queries.
func (ss *StatementSelect) Modify(modifiers ...func(s *sql.Selector)) *StatementSelect {
